...
def read_rows(path: Path) -> list[list[float]]:
    ...
    for line in path.read_text(encoding="utf-8", errors="replace").splitlines():
        ...
        fields = line.split()
        try:
            # TUM RGB/depth indexes pair a timestamp with a relative image path.
            # The audit only needs the timestamp and must not parse that path as a number.
            rows.append([float(fields[0])])
        except ValueError:
            continue
    return rows
...
def main() -> int:
    ...
    provenance = tum_state.get("archive_provenance", {})
    provenance = provenance if isinstance(provenance, dict) else {}
    official_length = tum_state.get("head", {}).get("content_length")
    official_length_matches = isinstance(official_length, int) and archive.exists() and archive.stat().st_size == official_length
    provenance_row = {"dataset_id": "TUM_FR1_ROOM", "archive_acquisition_mode": provenance.get("mode", "not_recorded"), "direct_official_download_by_this_task": provenance.get("direct_official_download_by_this_task", False), "official_head_content_length_bytes": official_length, "archive_size_matches_official_head": official_length_matches, "notes": provenance.get("notes", "")}
    ...
    replica_gate = replica.get("storage_gate", {})
    replica_heads = replica.get("download_part_heads", [])
    ...
    write_csv(results / "asset_provenance_summary.csv", list(provenance_row), [provenance_row])
    ...
    metrics = {"task": "Cross-Dataset Raw Asset Acquisition and Integrity Audit", "new_scientific_experiment_run": False, "baseline_navigation_run": False, "gsplat_training_run": False, "dataset_processing_run": False, "tum_dataset": "rgbd_dataset_freiburg1_room", "tum_official_source_verified": bool(tum_state.get("official_source_verified")), "tum_license_recorded": bool(tum_state.get("license_recorded")), "tum_download_attempted": bool(tum_state.get("download_attempted")), "tum_archive_acquisition_mode": provenance_row["archive_acquisition_mode"], "tum_direct_official_download_by_this_task": provenance_row["direct_official_download_by_this_task"], "tum_archive_size_matches_official_head": official_length_matches, "tum_download_completed": archive.exists(), "tum_archive_valid": archive_valid, "tum_archive_sha256": tum_archive_row["archive_sha256"], "tum_archive_size_bytes": tum_archive_row["archive_size_bytes"], "tum_extracted": tum_raw.is_dir(), "tum_rgb_count": len(rgb_files) if tum_raw.exists() else None, "tum_depth_count": len(depth_files) if tum_raw.exists() else None, "tum_groundtruth_row_count": len(gt_rows) if tum_raw.exists() else None, "tum_triple_match_rate_diagnostic": triple_rate, "tum_integrity_passed": tum_integrity, "replica_version": "v1", "replica_official_source_verified": bool(replica.get("commit")), "replica_license_recorded": bool(replica.get("license_sha256")), "replica_manual_acceptance_marker_present": marker_present, "replica_storage_status": replica_gate.get("replica_storage_status"), "replica_download_estimated_bytes": replica_gate.get("download_estimated_bytes"), "replica_extracted_estimated_bytes": replica_gate.get("estimated_extracted_bytes"), "replica_combined_space_requirement_bytes": replica_gate.get("combined_space_requirement_bytes"), "replica_storage_gate_passed": bool(replica_gate.get("storage_gate_passed")), "replica_download_attempted": False, "replica_download_completed": False, "replica_split_part_count_expected": 17, "replica_download_object_head_count": len(replica_heads), "replica_split_part_count_downloaded": 0, "replica_scene_count": None, "replica_complete_scene_count": None, "replica_integrity_passed": replica_integrity, "third_party_data_committed": False, "third_party_license_text_committed": False, "raw_trace_committed": False, "official_core_source_modified": False, "forbidden_paths_modified": False, "acquisition_decision": decision, "limitations": ["raw asset acquisition and integrity only", "no data preprocessing", "no Gaussian reconstruction", "no SAFER baseline execution", "no method-improvement module evaluation", "Replica download requires manual user acceptance of its research terms"]}
...
所有原始资产仅位于 `{root}`；自动下载仅允许官方 host，完成后至少保留 50 GiB。Replica 已对 `{len(replica_heads)}` 个官方下载对象执行 HEAD：下载估计 `{replica_gate.get('download_estimated_bytes')}` 字节，保守解压估计 `{replica_gate.get('estimated_extracted_bytes')}` 字节，合计空间需求 `{replica_gate.get('combined_space_requirement_bytes')}` 字节，存储门槛通过：`{replica_gate.get('storage_gate_passed')}`。
...
本次归档来源模式：`{provenance_row['archive_acquisition_mode']}`；本任务直接官网下载：`{provenance_row['direct_official_download_by_this_task']}`；归档大小与官方 HEAD 一致：`{official_length_matches}`。该归档由用户提供的本地文件传输至外部资产目录，未被表述为本任务直接官网下载。
...
记录官方 metadata、LICENSE 和 `download.sh` hashes；预期分卷为 17 个，连同附加配置共 `{len(replica_heads)}` 个官方对象。完整 Replica v1 仍只能按官方脚本获取。
