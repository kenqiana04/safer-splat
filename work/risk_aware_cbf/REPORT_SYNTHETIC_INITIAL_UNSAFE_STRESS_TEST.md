# Synthetic Initial-Unsafe Stress Test for FAS-CBF

## Scope

This report evaluates synthetic initial near-unsafe / unsafe starts for FAS-CBF. It is a reproduction-side stress test under `work/risk_aware_cbf/`. It does not modify official SAFER-Splat source code, does not claim a new CBF theorem, and is not an original benchmark result. Synthetic perturbed starts are reported separately from official starts.

## Motivation

V4-A active projection only had 8 naturally occurring repair-needed cases in dense flight official100. This synthetic stress test evaluates whether active-set verified safe-start projection generalizes beyond those natural cases.
...
If active-set projection succeeds on most synthetic unsafe cases, it supports the generality of FAS-Project beyond naturally occurring repair-needed starts. If it fails for large perturbations, the failure boundary should be reported rather than hidden. If V4-C reduces post-repair horizon margin violations, it supports using V4-C as a predictive recovery layer after safe-start repair. Runtime remains a practical limitation when V4-C is activated frequently.

Synthetic stress test results are not original benchmark results. Margin violations are not collisions. Collision-free and margin-satisfying behavior are reported separately.

## Claim boundary

No official SAFER-Splat source code is modified. No new CBF theorem is claimed. `min_safety_h` is not meter clearance; it is the repository's GSplat ellipsoid safety h value. Synthetic perturbed starts are not official benchmark starts. Post-repair navigation is not original benchmark navigation. V4-C is an empirical sampling-based predictive recovery wrapper unless formal assumptions are later proved.
