...
TUM 许可证已记录为 CC BY 4.0。Replica marker present: `True`；未由本任务创建或伪造。
...
Replica status: `raw_closure_passed`。marker 存在、非空且权限为 `600`；本任务未读取或输出其正文。

## 12. Replica Download and Scene Integrity
17 个分卷均按预期大小闭包并记录 SHA-256。合并归档为 `33859047808` 字节，tar 扫描通过（576 个成员，无绝对路径或路径遍历）。安全 staging 解压后原子提升至外部目录；18 个场景均具备 mesh、navmesh、textures 和场景元数据。未发现额外 Habitat 配置 ZIP，已明确记录为后续 renderer 配置门槛。
...
`tum_ready_replica_core_ready_additional_configs_missing`
