...
class ROSDataset(InputDataset):
    ...
    def __init__(
        ...
        self.pose2c = self.metadata["pose_to_camera"]
        ...
        if use_pc:
            assert "pc_topic" in dataparser_outputs.metadata.keys()
            self.pc_topic_name = self.metadata["pc_topic"]
            self.pc_dict = {}  # index -> point cloud
            if "pc_to_camera" in self.metadata:
                self.pc2c = self.metadata["pc_to_camera"]
