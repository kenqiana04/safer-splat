...
class ROSDataParser(DataParser):
    ...
    def _generate_dataparser_outputs(self, split="train", num_images: int = 500):
        ...
        if "pose_to_camera" in meta:
            pose_to_camera = torch.tensor(meta["pose_to_camera"], dtype=torch.float32)
        else:
            pose_to_camera = torch.eye(4, dtype=torch.float32)
        ...
        if "pc_topic" in meta:
            metadata["pc_topic"] = meta["pc_topic"]
            if "pc_to_camera" in meta:
                metadata["pc_to_camera"] = torch.tensor(meta["pc_to_camera"])
            else:
                metadata["pc_to_camera"] = torch.eye(4, dtype=torch.float32)

        metadata["scene_scale_factor"] = self.scene_scale_factor
