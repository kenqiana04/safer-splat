...
from validate_bounded_local_recovery_smoke_v1 import (
    LOCK, PROTOCOL, REPO, TASK, git, read, sha, validate_freeze,
)
from run_bounded_local_recovery_smoke_trial_v1 import AUTH_NAME, TOKEN_ENV

LAUNCH_MARKER = "BOUNDED_RECOVERY_SMOKE_LAUNCH_AUTHORIZATION.json"
...
def launch() -> None:
    ...
    write(root / LAUNCH_MARKER, {
        "schema": "BOUNDED_RECOVERY_SMOKE_LAUNCH_AUTHORIZATION_V1",
...
def batch_internal(token: str) -> int:
    ...
    if marker != {"schema": "BOUNDED_RECOVERY_SMOKE_LAUNCH_AUTHORIZATION_V1",
                   "token_sha256": hashlib.sha256(token.encode()).hexdigest(),
                   "protocol_sha256": sha(PROTOCOL), "execution_lock_sha256": sha(LOCK),
                   "source_head": git("rev-parse", "HEAD"),
    ...
    child_script = TASK / "run_bounded_local_recovery_smoke_trial_v1.py"
