...
def analyze(write_outputs=True):
    ...
    unknown=sum(x["hard_unknown_segments"] for x in hard); active_hard=sum(x["hard_violation_segments"]>0 for x in hard); active_only=sum(x["active_only_hard_discordant"] for x in rows); formal_hard_violation_count=sum(x["hard_violation_segments"] for x in hard)
    hard_zero={k:v["value"] if isinstance(v,dict) else v for k,v in audit.items()}; integrity=not any(hard_zero.values()) and formal_hard_violation_count==0 and recovery["l2_exception"]==recovery["l3_exception"]==0 and all(x.get("typed_boundary_contract_valid",True) for x in boundary_rows)
