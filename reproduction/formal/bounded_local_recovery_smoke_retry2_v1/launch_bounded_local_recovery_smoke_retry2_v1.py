...
from run_bounded_local_recovery_smoke_retry2_trial_v1 import AUTH_NAME, TOKEN_ENV

LAUNCH_MARKER = "BOUNDED_RECOVERY_SMOKE_RETRY2_LAUNCH_AUTHORIZATION.json"
...
def launch() -> None:
    ...
    write(root / LAUNCH_MARKER, {
        "schema": "BOUNDED_RECOVERY_SMOKE_RETRY2_LAUNCH_AUTHORIZATION_V1",
    ...
    print(json.dumps({"status": "FUTURE_SMOKE_LAUNCHED", "session": session,
...
def batch_internal(token: str) -> int:
    ...
    if marker != {"schema": "BOUNDED_RECOVERY_SMOKE_RETRY2_LAUNCH_AUTHORIZATION_V1",
                   "token_sha256": hashlib.sha256(token.encode()).hexdigest(),
                   "protocol_sha256": sha(PROTOCOL), "execution_lock_sha256": sha(LOCK),
                   "source_head": git("rev-parse", "HEAD"),
    ...
    child_script = TASK / "run_bounded_local_recovery_smoke_retry2_trial_v1.py"
...
def main() -> int:
    ...
    parser.add_argument("--authorize-execution", choices=("EXECUTE_BOUNDED_LOCAL_RECOVERY_SMOKE_RETRY2_V1",))
    parser.add_argument("--launch-token", help=argparse.SUPPRESS)
    args = parser.parse_args()
    if args.launch:
        if args.authorize_execution != "EXECUTE_BOUNDED_LOCAL_RECOVERY_SMOKE_RETRY2_V1":
