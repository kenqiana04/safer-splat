...
def main()->int:
    ap=argparse.ArgumentParser(); ap.add_argument("--checkout",type=Path,required=True); ap.add_argument("--result-root",type=Path,required=True); ap.add_argument("--map-source-root",type=Path,required=True); a=ap.parse_args()
    checkout=a.checkout.resolve(strict=True); result=a.result_root.resolve(strict=True); task=Path(__file__).resolve().parent
    runner=import_file(task/"run_active_runtime_v3_paired_validation.py","_paired_runner")
