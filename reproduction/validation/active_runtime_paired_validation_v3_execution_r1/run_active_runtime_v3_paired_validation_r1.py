...
DATA_SOURCE = Path("/disk1/zlab/datasets/safer_splat_gdrive/data/stonehenge")
OUTPUT_SOURCE = Path("/disk1/zlab/projects/safer-splat/outputs/stonehenge")
...
def ensure_path_plumbing(checkout: Path) -> dict[str, Any]:
    ...
    for target, source in sources.items():
        ...
        if os.path.lexists(target):
            if not target.is_symlink() or target.resolve() != source:
                raise RuntimeError("RELATIVE_PATH_TARGET_WRONG:" + str(target))
        else:
            target.parent.mkdir(parents=True, exist_ok=True)
            target.symlink_to(source, target_is_directory=True)
        if target.resolve() != source:
            raise RuntimeError("RELATIVE_PATH_TARGET_MISMATCH:" + str(target))
    transforms = checkout / "data/stonehenge/transforms.json"
    ...
    return {"data_target": str(checkout / "data/stonehenge"), "output_target": str(checkout / "outputs/stonehenge"),
            "transforms": str(transforms), "checkpoint": str(checkpoint), "checkpoint_sha256": CHECKPOINT_SHA}

def verify_repaired_source(checkout: Path) -> dict[str, Any]:
    if subprocess.run(["git", "-C", str(checkout), "merge-base", "--is-ancestor", REPAIRED_HEAD, "HEAD"]).returncode:
        raise RuntimeError("REPAIRED_HEAD_NOT_BASELINE")
    if git(checkout, "rev-parse", f"{REPAIRED_HEAD}^") != REPAIRED_PARENT:
        raise RuntimeError("REPAIRED_PARENT_MISMATCH")
    hashes = {}
    for rel in ("reproduction/runtime/active_runtime_assurance_v2/active_cycle.py",
                "reproduction/runtime/active_runtime_assurance_v2/supervisor.py"):
        hashes[rel] = sha256_file(checkout / rel)
        committed = subprocess.run(["git", "-C", str(checkout), "show", f"{REPAIRED_HEAD}:{rel}"], capture_output=True, check=True).stdout
        if hashlib.sha256(committed).hexdigest() != hashes[rel]:
            raise RuntimeError("REPAIRED_RUNTIME_SOURCE_DRIFT:" + rel)
    return {"head": REPAIRED_HEAD, "parent": REPAIRED_PARENT, "repair_pr": REPAIR_PR,
...
def verify_execution_lock(checkout: Path, require_committed: bool) -> dict[str, Any]:
    ...
    if line != f"{sha256_file(LOCK_PATH)}  V3_PAIRED_EXECUTION_R1_LOCK.json":
    ...
    if lock.get("execution", {}).get("result_root") != str(DEFAULT_RESULT_ROOT):
        raise RuntimeError("EXECUTION_RESULT_ROOT_MISMATCH")
    for name in lock["harness_file_sha256"]:
...
def static_preflight(checkout: Path, map_root: Path) -> dict[str, Any]:
    verify_repaired_source(checkout)
    ...
    plumbing = ensure_path_plumbing(checkout)
    verify_fresh_result_root(DEFAULT_RESULT_ROOT, False)
    ...
    return {"status": "PASS", "parent": PARENT, "protocol_sha256": PROTOCOL_SHA, "map_artifacts": artifacts,
            ...
            "execution_counts": counts, "protected_diff": 0, "repaired_source": verify_repaired_source(checkout),
            "path_plumbing": plumbing, "future_result_root_absent": True}
...
def summarize_integrity(output_dir: Path) -> dict[str, Any]:
    ...
    result = {"schema": "ACTIVE_RUNTIME_V3_PAIRED_COLLECTION_INTEGRITY_SUMMARY_R1_V1", "completed_trials": len(complete),
    ...
    write_json(output_dir / "ACTIVE_V3_R1_COLLECTION_INTEGRITY_SUMMARY.json", result)
