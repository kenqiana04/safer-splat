...
PREVIOUS_HEAD = "b8c64950770c52a89f073ef47b01c84d79dd9dbf"
...
RESULT_ROOT = Path("/disk1/zlab/v3_execution_records/active_runtime_paired_validation_v3_r1_retry1_20260915")
...
def cpu_test(task: Path, name: str) -> bool:
    return subprocess.run([sys.executable, str(task / name), "--task-dir", str(task)],
...
def main() -> int:
    ...
    need(subprocess.run(["git", "-C", str(repo), "merge-base", "--is-ancestor", PREVIOUS_HEAD, "HEAD"]).returncode == 0, "PR147_PREVIOUS_HEAD_ANCESTOR", failures)
    ...
    need(lock.get("repaired_runtime_head") == REPAIRED_HEAD and lock.get("repaired_runtime_parent") == REPAIRED_PARENT and lock.get("repair_pr") == 146 and lock.get("r1_harness_repair_parent_head") == PREVIOUS_HEAD, "REPAIR_IDENTITY", failures)
    ...
    need(OLD_FAILED_RESULT_ROOT.is_dir(), "OLD_FAILED_RESULT_ROOT_PRESERVED", failures)
    need((OLD_FAILED_RESULT_ROOT / "raw/gpu_preflight.json").is_file() and
         (OLD_FAILED_RESULT_ROOT / "launcher.log").is_file() and
         not (OLD_FAILED_RESULT_ROOT / "raw/trial_66").exists(), "OLD_FAILED_ATTEMPT_DIAGNOSTIC_FACTS", failures)
    ...
    need(all(flag in runner for flag in ("--static-preflight","--gpu-preflight","--one","--batch","--summarize-integrity","--resume","--batch-child")), "RUNNER_MODES", failures)
    need("ACTIVE_V3_R1_COLLECTION_INTEGRITY_SUMMARY" in runner and "complete_evidence" in runner, "INTEGRITY_SUMMARY", failures)
    need("SAFER_SPLAT_R1_BATCH_CHILD_TOKEN" in runner and "R1_BATCH_CHILD_AUTHORIZATION.json" in runner and
         "BATCH_CHILD_RESULT_ROOT_MISMATCH" in runner and "BATCH_CHILD_PARENT_NOT_ALIVE" in runner,
         "NARROW_BATCH_CHILD_AUTHORITY", failures)
    need("parent_failures" in runner and "EARLY_CHILD_FAILURE.json" in runner and
         "immutable_trial_evidence_lock_created\": False" in runner and "HARD_STOP_NO_RETRY" in runner,
         "EARLY_CHILD_FAILURE_PRESERVATION", failures)
    ...
    need(str(RESULT_ROOT) in launcher and str(OLD_FAILED_RESULT_ROOT) not in launcher, "RETRY1_ROOT_ONLY", failures)
    ...
    need(str(RESULT_ROOT) in monitor and str(OLD_FAILED_RESULT_ROOT) not in monitor, "MONITOR_RETRY1_ROOT", failures)
    ...
    history = lock.get("failed_attempt_history", {})
    need(history.get("result_root") == str(OLD_FAILED_RESULT_ROOT) and history.get("disposition") == "READ_ONLY_DIAGNOSTIC_EVIDENCE_SUPERSEDED_NO_REUSE" and history.get("gpu_preflight_count") == 1 and history.get("scientific_trial_count") == 0, "FAILED_ATTEMPT_HISTORY", failures)
    repair_counts = lock.get("repair_task_execution_counts", {})
    need(all(repair_counts.get(k) == 0 for k in ("gpu_preflight","active_scientific_trial","scientific_analyzer","reference_rerun","official100","formal_new_outcome")), "REPAIR_TASK_ZERO_EXECUTION", failures)
    need(cpu_test(task, "test_first_launch_preflight_order_r1.py"), "FIRST_LAUNCH_REGRESSION", failures)
    need(cpu_test(task, "test_batch_child_root_and_parent_partial_evidence_r1.py"), "CHILD_ROOT_AND_EARLY_FAILURE_REGRESSION", failures)
    if failures:
