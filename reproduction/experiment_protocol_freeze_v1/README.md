...
python reproduction/experiment_protocol_freeze_v1/scripts/collect_cross_dataset_prefreeze_provenance.py
...
The validator performs no experiment and must be re-run after any frozen artifact changes. It separates historical freeze validity from Cross-Dataset G0/G1 readiness. Re-run `build_freeze_bundle.py` immediately before validation so the manifest excludes only itself and matches all other files.
